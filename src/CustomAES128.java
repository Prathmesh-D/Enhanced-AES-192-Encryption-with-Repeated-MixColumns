...
public class CustomAES128 {
    private static final int BLOCK_SIZE = 16; // AES block size in bytes
    private static final int KEY_SIZE = 16; // AES key size in bytes (128 bits)
    private static final int NUM_ROUNDS = 10; // Number of rounds for AES-128
    ...
    private static final byte[] R_CON = new byte[]{
        (byte) 0x8d, (byte) 0x01, (byte) 0x02, (byte) 0x04, (byte) 0x08, (byte) 0x10, (byte) 0x20, (byte) 0x40,
        (byte) 0x80, (byte) 0x1b, (byte) 0x36
    };

    public CustomAES128(byte[] key) {
        this.key = key;
        this.roundKeys = new byte[NUM_ROUNDS + 1][BLOCK_SIZE];
        keyExpansion();
    }
    ...
    while (i < BLOCK_SIZE * (NUM_ROUNDS + 1)) {
        for (int j = 0; j < 4; j++) {
            temp[j] = roundKeys[(i - 1) / KEY_SIZE][(i - 1) % KEY_SIZE];
        }
        if (i % KEY_SIZE == 0) {
            byte t = temp[0];
            temp[0] = (byte) (S_BOX[temp[1] & 0xFF] & 0xFF);
            temp[1] = (byte) (S_BOX[temp[2] & 0xFF] & 0xFF);
            temp[2] = (byte) (S_BOX[temp[3] & 0xFF] & 0xFF);
            temp[3] = (byte) (S_BOX[t & 0xFF] & 0xFF);
            temp[0] = (byte) (temp[0] ^ R_CON[i / KEY_SIZE]);
        } else if (KEY_SIZE > 6 && i % KEY_SIZE == 4) {
            for (int j = 0; j < 4; j++) {
                temp[j] = (byte) (S_BOX[temp[j] & 0xFF] & 0xFF);
            }
        }
        for (int j = 0; j < 4; j++) {
            roundKeys[i / KEY_SIZE][i % KEY_SIZE] = (byte) (roundKeys[(i - KEY_SIZE) / KEY_SIZE][(i - KEY_SIZE) % KEY_SIZE] ^ temp[j]);
            i++;
        }
    }
}
    ...
    private void mixColumns(byte[] state) {
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 4; j++) {
                int s0 = state[j * 4] & 0xFF;
                int s1 = state[j * 4 + 1] & 0xFF;
                int s2 = state[j * 4 + 2] & 0xFF;
                int s3 = state[j * 4 + 3] & 0xFF;

                int h = s0 ^ s1 ^ s2 ^ s3;
                state[j * 4] ^= h ^ xtime(s0 ^ s1);
                state[j * 4 + 1] ^= h ^ xtime(s1 ^ s2);
                state[j * 4 + 2] ^= h ^ xtime(s2 ^ s3);
                state[j * 4 + 3] ^= h ^ xtime(s3 ^ s0);
            }
        }
    }

    private byte xtime(int x) {
        return (byte) (((x << 1) & 0xFF) ^ (((x >> 7) & 1) * 0x1b));
    }
    ...
    public static void main(String[] args) {
        ...
        String customKey = generateRandomKey(); // Randomm Key Generate
        ...
        CustomAES128 aes = new CustomAES128(key);
        ...
        System.out.println("Custom Plaintext: " + customPlaintext);
